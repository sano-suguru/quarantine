...
The client (`game/net/client.ts` `onSnap`, which already computes `prevPhase → phase` and calls `siegeEdgeCue`) reacts to **two distinct edges**:

- **`night → breached`:** show the fallen-fortress cue (banner + dread audio) via `siegeEdgeCue`. **Do NOT clear the buffer** — keep interpolating the frozen tableau (downed bodies, stalled zombies) for the beat's duration. This is the horror payoff.
- **`breached → resetting`:** run the existing **`resetNet`** (`client.ts` — reused verbatim from reconnect; it clears `buf`, `prev`, `prevPhase`, ghosts, prediction/`predInit`, `lastTick`). Because `effects()` is guarded by `if (this.prev)`, nulling `prev` suppresses the diff-derived fx (the mass `fxKill`/`fxImpact` the wholesale id churn would otherwise fire) for the reset; `prevPhase = null` suppresses a spurious siege banner on the first post-reset snapshot (the same guarantee drop-in relies on). The reset-frame snapshot then becomes the new `prev`; `render()` early-returns on the now-empty `buf` for at most one frame (imperceptible) until the Day-1 snapshots refill it. The `resetting` cue itself is a **no-op**.

- **The reset trigger is the phase edge, not `day → 1`.** A breach on the **night of day 1** leaves `day` at 1→1 (no change), so a `day`-diff trigger would miss it; the `→ resetting` edge is unambiguous. (`day → 1` may serve as a secondary assertion only.)
