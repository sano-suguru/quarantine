...
- 対象: `game/game.ts`・`game/main.ts`・`game/net/*.ts`（`events.ts`/`net.ts`/`client.ts`/`link.ts`/`ghost.ts`）・`game/systems/stalkerFx.ts`・`game/input.ts`・`game/ui.ts`（実測 ~25-30箇所）。
- ⚠ 各コメントを実コードと突き合わせる。`host & client` が「現在 client のみだが将来のため」ではなく「今 client だけ」なら client 語彙へ。`single-player stays byte-for-byte` のような「もう存在しないモードの保証」は削除ないし「client/DO で一貫」へ書き換え。
...
- `game/net/net.ts` の `NetMode` 型と `Net.mode` フィールドを削除。
...
- ⚠ `Net.mode` を読む全箇所を grep で洗い、漏れなく無条件化してから型/フィールドを消す。
...
短い spec（本書）→ rubber-duck 盲点レビュー（実 netcode/DO/CF 接地）→ writing-plans → SDD（`.superpowers/sdd/progress.md` に ledger）→ PR → CI（check + worker 両 required）→ マージ。実装は `feat/do-2b3-cleanup`（main から）。デプロイ（deploy-worker.yml 手動）は **2b③ 後**（ユーザ確定済み）。
