...
- **1 入力サンプルあたり最大 1 ステップにクランプ**：`Input.wheel` に溜まったノッチ合計の符号（`Math.sign`）で 1 ステップだけ進める。加えて**切り替え直後に短いクールダウン**（プレイテストで詰める、初期値 ~120ms 目安）を設け、その間のホイール入力は消費するが切り替えは行わない。これはトラックパッドの慣性スクロールや高精細ホイールが 1 回の物理操作で `wheel` イベントを多発させ、指 1 本のスワイプで武器を一周してしまう事故を防ぐため（feel-first）。
...
- canvas の `wheel` イベントを **`{ passive: false }` を明示指定**して登録し、ハンドラ内で `this.wheel += Math.sign(e.deltaY)`、`e.preventDefault()`（ページスクロール抑制）。canvas 要素へのバインドなので input/textarea 上のホイールはそもそも届かず、テキスト入力フォーカス時の誤作動は自然に回避される。
...
- クールダウン中でなく `w !== 0` なら `cycleWeaponSlot(WEAPON_ORDER, (id) => !!state.owned[id] && !WEAPONS[id]?.melee, p.weapon, Math.sign(w))` を呼び、結果（非 `null`）を `weaponSlot` に代入して切り替えクールダウンを起動。
- クールダウンは `localInput.ts` のモジュールローカル状態（`prevKeys` / `aimTargetId` と同様）で保持。時間源はフレームカウンタ方式（sim ステップ非依存に、実時間ベースにするなら `performance.now()` を localInput 内で使用可 — ここは DOM 境界なので可。ただし実装時に既存慣習と整合する方を選ぶ）。
- `emptyInput()` を返す早期 return 経路（`p.hp<=0` 等）に入る前に `Input.wheel = 0` をリセットして、非ライブ中の蓄積を捨てる。
