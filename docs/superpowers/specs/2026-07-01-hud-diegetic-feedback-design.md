...
2. **Dimming must not make the whole game feel dark from the start.** The steady level stays at full above an onset charge (`dimStart`) and from there eases *linearly* down toward `dimFloor` as the battery empties — a fresh battery looks bright and the weakening reads as a gradual *progression* toward the dying, deep-flickering low state. A linear ramp (not a smoothstep) is the chosen starting shape: it is simple, unit-testable, and gives the whole mid-range a readable cue (the point of replacing the bar), while the onset is gentle enough near full charge that it never feels "dark from the start." The exact onset and shape are a feel detail tuned at playtest.

New `CONFIG.flashlight` knobs: `dimFloor` (the steady floor) and `dimStart` (the onset charge above which the cone is full-bright). Floor, onset, and curve are **tuned in CONFIG and validated by playtest** — feel-first; not done until felt.
