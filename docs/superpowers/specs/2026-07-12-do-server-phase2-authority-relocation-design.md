...
Post-big-bang, **no browser calls `update()` — only the DO does.** The browser client is pure predict+interpolate and already runs `sysFx`/`sysCamera` itself in its frame path (`main.ts:468–470`). So excluding the cosmetic *systems* from authority is a straightforward removal (not a new fork), though a cosmetic *residue* remains — see the `cam.shake` caveat:
