...
`feel.ts` splits the same way: recoil/muzzle/shake **numbers** stay pure state mutation; the `Audio.shot`/`melee` cue becomes a `muzzle` event. The old dual path (host direct-call vs client `effects()` diffing) **collapses into the single buffer→drain path**, removing the existing host/client fx divergence.

**Client-only systems (corrected):** `stalkerFx`/`stalkerPhantom` are driven from `draw()` (not `update()`) and import `Audio` — they stay in `game/`. But `sysFx` (particle advance) and `sysCamera` (per-client camera) **are** called inside `update()` today (`game.ts:188,190`); they are pure, so they can live in `sim/`, but they are cosmetic / per-client and the **DO must not run them** — excluding them from the DO's `update()` path (and moving `state.cam`/particles off the authoritative sim) is **Phase-2 work**, flagged now as a latent issue. Phase 1 leaves them in `update()` so single-player feel is unchanged.
